...
format: 
  html:
    ...
    theme: cosmo
    page-layout: full
    css: styles.css # optional, only if you have a custom CSS
...
The merged dataset contains the following key fields:
- **country**: Name of the country
- **iso_alpha**: ISO country code (3-letter)
- **year_x**: Year associated with hygiene coverage data
- **hygiene_coverage**: Proportion of healthcare facilities with basic hygiene
- **year_y**: Year associated with the socioeconomic data
- **population, total**: Total national population
- **gdp per capita (constant 2015 US$)**: Economic prosperity indicator (may contain missing values)
- **gni (current US$)**: Gross National Income, though often missing (NaN) for earlier years

It is important to note that the dataset contains for key economic indicators like GDP per capita and GNI, especially for earlier years. This may require additional data cleaning, imputation, or cautious interpretation when conducting further analysis.
